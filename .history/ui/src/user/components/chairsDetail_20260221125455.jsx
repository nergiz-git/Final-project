...
const ChairsDetail = ({ favorites, basket, dispatch }) => {
...
const infavorite = favorites?.find(a => a && String(a.id) === String(product?.id) && a.category === "chairs");
const inBasket = basket?.find(a => a && String(a.id) === String(product?.id) && a.category === "chairs");
...
const handleAddToFav = () => {
    ...
    if (infavorite) {
        const updated = favorites.filter(a => a && String(a.id) !== String(product.id)&& a.category === "chairs");
        toastTriggered.current.removeFav = true;
        dispatch({ type: "SET_Favorites", payload: updated });
        localStorage.setItem("favorites", JSON.stringify(updated));
    } else {
        // const updated = [...favorites, { id: product.id, name: product.name, category: "chairs" }];
         const updated = [...favorites, { id: product.id, name: product.heading || product.name, category: "chairs" }];
...
const handleAddToCart = () => {
    ...
    if (inBasket) {
        const updated = basket.filter(a => String(a.id) !== String(product.id)&& a.category === "chairs");
        toastTriggered.current.removeCart = true;
        dispatch({ type: "SET_BASKET", payload: updated });
        localStorage.setItem("basket", JSON.stringify(updated));
    } else {
        const updated = [...basket, { id: product.id, count: 1, category: "chairs" }];
...
export default connect(mapStateToProps)(ChairsDetail);