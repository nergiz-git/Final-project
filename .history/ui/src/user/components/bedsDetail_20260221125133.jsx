...
const BedsDetail = ({ favorites, basket, dispatch }) => {
  ...
  document.body.className = theme ? "dark" : "";
    ...
    const infavorite = favorites?.find(a => a && String(a.id) === String(product?.id) && a.category === "beds");
    const inBasket = basket?.find(a => a && String(a.id) === String(product?.id) && a.category === "beds");
    ...
    const handleAddToFav = () => {
        ...
        if (infavorite) {
            const updated = favorites.filter(a => a && String(a.id) !== String(product.id)&& a.category === "beds");
            toastTriggered.current.removeFav = true;
            dispatch({ type: "SET_Favorites", payload: updated });
            localStorage.setItem("favorites", JSON.stringify(updated));
        } else {
            // const updated = [...favorites, { id: product.id, name: product.name, category: "beds" }];
    ...
    const handleAddToCart = () => {
        ...
        if (inBasket) {
            const updated = basket.filter(a => String(a.id) !== String(product.id)&& a.category === "beds");
            toastTriggered.current.removeCart = true;
            dispatch({ type: "SET_BASKET", payload: updated });
            localStorage.setItem("basket", JSON.stringify(updated));
        } else {
            const updated = [...basket, { id: product.id, count: 1, category: "beds" }];
...
export default connect(mapStateToProps)(BedsDetail);