...
function CartPage({ products, sofas,chairs,beds,tables, basket, dispatch }) {
  useEffect(() => {
    document.title = "Cart";
  }, []);

   const allProducts = [...products, ...sofas,  ...chairs, ...tables, ...beds,];
  ...
  if (product)  {const price = parseFloat(String(product.price).replace(/,/g, ""));
    subtotal += price * basketItem.count;
  }
});
              ...
              {allProducts.length ? (
                basket.length ? (
                  basket.map((basketItem) => {
                    ...
                    const price = parseFloat(String(product.price).replace(/,/g, ""));
const total = price * basketItem.count;
                    return (
                      <div className="card-col" key={basketItem.id}>
                        <div className="cart">
                          <div className={`cart-img ${product.category === 'chairs' ? 'chairs-img' : ''}`}>
