...
const addUser = (e) => {
    ...
    if (user.name.trim() !== "" && user.email !== "" && user.pass !== "") {
        if (!validatePassword(user.pass)) {
            toast.error("Şifrə ən azı 6 simvol olmalı, 1 böyük hərf və 1 xüsusi simvol olmalıdır", {
                position: "top-center",
                autoClose: 5000,
                theme: "colored",
                transition: Bounce,
        ...
        setUser({ name: "", email: "", pass: "" });
        toast.success("Qeydiyyat uğurla tamamlandı", { position: "top-center", autoClose: 3000, theme: "light", transition: Bounce });

        setTimeout(() => setAddClass("left-panel-active"), 3000);
        setBool(false);
    } else {
        toast.error("Məlumatları tam daxil edin", { position: "top-center", autoClose: 5000, theme: "colored", transition: Bounce });
    }
};
   ...
   const validatePassword = (password) => {
    // Minimum 6 simvol, 1 böyük hərf, 1 xüsusi simvol
    const regex = /^(?=.*[A-Z])(?=.*[!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?]).{6,}$/;
