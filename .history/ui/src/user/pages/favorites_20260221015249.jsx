...
function Favorites({ products, sofas, favorites, basket, dispatch }) {
  const color = "#000";
  const loading = true;
  const override = {
    display: "block",
  ...
  const allProducts = [...products, ...sofas,...collectionData];
...
"
          ...
          filteredFavorites.length ? (
            <div className="favorite">
              {filteredFavorites.map((favorite) => {
                ...
                //   (p) => String(p.id) === String(favorite.id)
                // );
                const product = allProducts.find(
  ...
  alt={product.name || product.title}  className="mt-[50px]"
/>
                      <span onClick={() => removefavorit(favorite.id, favorite.category)}
                        style={{
                          top: product.category === 'sofa' ? '-85px' : '10px'  // sofa olarsa 30px, yoxsa 10px
                        }}
                      >
                        <VscClose />
                      </span>
                    ...
                    <div className="favorite-text">
                      ...
                      <div className="fov-titl">
                        ...
                        {inBasket ? (
                          <button
                            onClick={() => favoritremovecard(favorite.id, favorite.categor)}
                          ...
                          <button
                            onClick={() => favorittocard(favorite.id, favorite.categor)}
                            className="fovorite-btn"
                          >
                            <HiShoppingBag />
                          </button>
