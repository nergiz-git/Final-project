...
function Sign() {
  ...
  const addUser = (e) => {
    ...
    if (user.name.trim() === "" || user.email.trim() === "" || user.pass.trim() === "") {
        ...
        return; // Fetch çağırılmayacaq
    }

    // 2️⃣ Şifrə validasiyasını yoxla
    if (!checkPassword(user.pass)) {
        setShowPasswordRules(true); // Input altında qaydaları göstər
        setPasswordValid(false);    // Valid olmadığını qeyd et
        toast.error("Şifrə ən azı 6 simvol, 1 böyük hərf və 1 xüsusi simvol olmalıdır", {
            position: "top-center",
            autoClose: 5000,
            theme: "colored",
            transition: Bounce,
    ...
    // 3️⃣ Əgər hər şey düzgündür → qeydiyyat
    ...
    setTimeout(() => setAddClass("left-panel-active"), 3000);
};
                    ...
                    <div className={`form-container ${signUp} `}>
                        <form onSubmit={addUser}>
                            ...
                            {showPasswordRules && !passwordValid && (
                                <div className="text-red-500 text-[13px] rounded-xl mt-2 w-full max-w-[330px] mr-[85px]">
