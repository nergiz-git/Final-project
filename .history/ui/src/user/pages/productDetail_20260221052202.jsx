...
const ProductDetail = ({ favorites, basket, dispatch }) => {
    ...
    const infavorite = favorites?.find(a => a && String(a.id) === String(product?.id) && a.category === "sofas");
const inBasket = basket?.find(a => a && String(a.id) === String(product?.id) && a.category === "sofas");
    ...
    if (infavorite) {
        const updated = favorites.filter(a => !(String(a.id) === String(product.id) && a.category === "sofas"));
        toastTriggered.current.removeFav = true;
        dispatch({ type: "SET_Favorites", payload: updated });
        localStorage.setItem("favorites", JSON.stringify(updated));
    } else {
        const updated = [...favorites, { id: product.id, name: product.name, category: "sofas" }];
        toastTriggered.current.addFav = true;
        dispatch({ type: "SET_Favorites", payload: updated });
        localStorage.setItem("favorites", JSON.stringify(updated));
    }
