...
function FurnitureChairs() {
  const { search } = useOutletContext();
  const [chairs, setChairs] = useState([]);
  const [selectedOptions, setSelectedOptions] = useState({});
  const filteredChairs = chairs.filter(chair =>
    chair.title.toLowerCase().includes(search.toLowerCase())
  );

  const dataToShow = search ? filteredChairs : chairs;
  ...
  return (
    <div className={`pt-[1px] ${theme ? "bg-dark" : "bg-light"}`}>
      ...
      <div className="mt-32 sm:mt-40 md:mt-[200px] mx-4 sm:mx-6 md:mx-[48px] mb-8 sm:mb-12 md:mb-[68px]">
        <h1 className="text-3xl sm:text-4xl md:text-[50px] mb-2 sm:mb-3 md:mb-[10px] font-bold">Chairs</h1>
        <span className="group flex items-center gap-2 cursor-pointer text-sm sm:text-base">
          ...
          Discover chair design collections
        </span>
      </div>
      ...
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 md:px-8 lg:px-[40px] py-6 sm:py-8">
        ...
        <div className="block md:hidden">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
            ...
            {!search && (
              <div className="col-span-full h-[250px] sm:h-[350px]">
                <img src="https://cdn.media.amplience.net/i/boconcept/14c0abd4-d4d2-4e29-8950-b2f8008b975e?locale=*&w=1920&fmt=auto&upscale=false&sm=c&qlt=75&h=1440&poi=0.4375%2C0.45313963%2C0.125%2C0.09372071&scaleFit=poi" className="w-full h-full object-cover rounded-lg" alt="Chair" />
              </div>
            )}
        ...
        <div className="hidden md:grid grid-cols-4 gap-6 auto-rows-[350px]">
          ...
          {!search && (
            <div className="col-span-2 row-span-2">
               <img src="https://cdn.media.amplience.net/i/boconcept/194bc012-2d38-40ec-a983-b2f8008d9101?locale=*&w=1920&fmt=auto&upscale=false&sm=c&qlt=75&h=1440&poi=0.4444684%2C0.51714224%2C0.125%2C0.09372071&scaleFit=poi" className="w-full h-full object-cover" alt="Chair" />
            </div>
          )}

          {/* 2nd row right */}
          {dataToShow.slice(4, 6).map(product => (
            <ProductCard key={product.id} product={product} category="chairs"
...
export default FurnitureChairs;
